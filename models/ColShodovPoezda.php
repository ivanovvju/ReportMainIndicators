...
/**
 * Показатель "Количество сходов железнодорожного подвижного состава (при поездной работе) по вине ЦД".
 */
class ColShodovPoezda extends BaseIndicator
{
    /**
    ...
    {
        Log::Info("Формирование показателя: Количество сходов железнодорожного подвижного состава (при поездной работе) по вине ЦД");
    ...
    {
        if ($curl = curl_init()) {
            curl_setopt($curl, CURLOPT_URL, "http://effect.gvc.oao.rzd/effect/table/ARB_H207.xml?DAT={$this->utilsDate->year}.{$this->utilsDate->month}.{$this->utilsDate->day}");
            curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($curl, CURLOPT_COOKIE, "login=smenareq;password=smenareq");
            $out = curl_exec($curl);
            file_put_contents("tmp/ARB_H207_{$this->utilsDate->year}.xml", $out);
            $this->sisContents = $out;
            curl_close($curl);
            Log::Info("Скачали файл из СИС-Эффект: tmp/ARB_H207_{$this->utilsDate->year}.xml");
        }
    }
    ...
    {
        $dataFile = file_get_contents("tmp/ARB_H207_{$this->utilsDate->year}.xml");
